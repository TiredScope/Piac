class_name BME280Module
extends Module

const ID: String = "bme280"
...
func _on_message(m: Message) -> void:
	if m.type == Message.Type.M_BME280_VALUES:
		...
		values.pressure = reader.get_f32()
...
func set_reporting_delay(delay: int, discriminator: int = Message.DEFAULT_DISCRIMINATOR) -> void:
	var builder: MessageBuilder = MessageBuilder.new(Message.Type.M_BME280_SET_REPORTING_DELAY, discriminator)
